...
function Admin() {
  const { language, logoUrl, setLogoUrl, setLogoAssets } = useStore();
  const revenue = products.reduce((sum, product) => sum + product.priceJod * Math.max(3, 60 - product.inventory), 0);
  ...
  return (
    <main className="page page-enter">
      <PageTitle eyebrow={t(language, "operations")} title={t(language, "adminTitle")} copy={t(language, "adminCopy")} />
      <section className="admin-metrics">
        <Metric icon={<BarChart3 />} label={t(language, "revenueForecast")} value={`${revenue.toLocaleString()} JOD`} />
        <Metric icon={<ShoppingBag />} label={t(language, "orders")} value="148" />
        <Metric icon={<Gauge />} label={t(language, "conversion")} value="4.8%" />
        <Metric icon={<ShieldCheck />} label={t(language, "fraudChecks")} value="100%" />
      </section>
      <section className="glass logo-admin">
        <div><h2>{t(language, "logoManagement")}</h2><p>{t(language, "logoManagementCopy")}</p></div>
        <div className="logo-preview"><span>{t(language, "currentLogo")}</span><img src={logoUrl} alt={t(language, "logoAlt")} /></div>
        <label className="primary-button upload-button"><Upload size={18} /> {t(language, "uploadLogo")}<input type="file" accept="image/*" onChange={(event) => uploadLogo(event.target.files?.[0])} /></label>
        <button className="secondary-button" onClick={() => setLogoAssets("/brand/shakra-logo-full.png", "/brand/shakra-logo-icon.png")}>{t(language, "resetLogo")}</button>
      </section>
      <section className="glass admin-table">
        <h2>{t(language, "inventoryManagement")}</h2>
        <table><thead><tr><th>{t(language, "product")}</th><th>{t(language, "collection")}</th><th>{t(language, "stock")}</th><th>{t(language, "status")}</th></tr></thead><tbody>{products.map((product) => <tr key={product.id}><td>{lp(language, product).name}</td><td>{lc(language, product.collection)}</td><td>{product.inventory}</td><td>{product.inventory < 25 ? t(language, "reorder") : t(language, "healthy")}</td></tr>)}</tbody></table>
      </section>
    </main>
  );
}
...
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="glass metric">{icon}<span>{label}</span><strong>{value}</strong></div>;
}
